...
@if($activity->icon ?? null)
    ...
    @if($svg)
        <span class="{{ $class }} flex-shrink-0 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full [&>svg]:object-contain" title="{{ $activity->translate('title') ?? $activity->title }}">{!! $svg !!}</span>
    @else
        {{-- Use object tag with explicit type to force SVG rendering (avoids Content-Type issues) --}}
        <object data="{{ $activity->icon_url }}" type="image/svg+xml" class="{{ $class }} flex-shrink-0" title="{{ $activity->translate('title') ?? $activity->title }}" aria-hidden="true">

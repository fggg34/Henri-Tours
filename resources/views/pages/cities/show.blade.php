...
@section('title', $city->translate('name') . ' - ' . config('app.name'))
@section('description', Str::limit(strip_tags($city->translate('description') ?? ''), 160))

@push('meta')
<meta property="og:title" content="{{ $city->translate('name') }}">
<meta property="og:description" content="{{ Str::limit(strip_tags($city->translate('description') ?? ''), 200) }}">
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
    ...
    <nav class="text-sm text-gray-500 mb-8" aria-label="Breadcrumb">
        <ol class="flex items-center gap-1.5">
            ...
            <li>/</li>
            <li><a href="{{ localized_route('cities.index') }}" class="text-brand-navy hover:text-brand-navy transition">Cities</a></li>
            <li>/</li>
            <li class="text-gray-700">{{ $city->translate('name') }}</li>
        </ol>
    </nav>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 mb-14">
            ...
            @if($allImages->isNotEmpty())
                <div class="relative rounded-2xl overflow-hidden bg-gray-200" style="aspect-ratio: 16/10;">
                    <a href="{{ $allImages[0] }}" class="glightbox block w-full h-full" data-gallery="city-gallery">
                        <img src="{{ $allImages[0] }}" alt="{{ $city->translate('name') }}" class="w-full h-full object-cover">
                    </a>
                    @if($totalPhotos > 1)
                    ...
                    <div class="grid grid-cols-4 gap-2 mt-2">
                        @foreach($thumbImages->take(4) as $url)
                            <a href="{{ $url }}" class="glightbox group block aspect-[4/3] rounded-xl overflow-hidden bg-gray-200" data-gallery="city-gallery">
                                <img src="{{ $url }}" alt="{{ $city->translate('name') }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy">
                            </a>
                        @endforeach
                    </div>
                @endif
        ...
        <div class="flex flex-col justify-center">
            @if($city->translate('country') ?: $city->country)
                <p class="text-xs font-medium uppercase tracking-wider text-brand-navy mb-2">{{ $city->translate('country') ?: $city->country }}</p>
            @endif
            <h1 class="text-3xl md:text-4xl font-bold text-gray-900 mb-5">{{ $city->translate('name') }}</h1>
            @if($city->translate('description'))
    ...
    <section class="mb-14 overflow-hidden">
        <div class="flex items-end justify-between mb-8">
            <div>
                ...
                <h2 class="text-2xl md:text-3xl font-bold text-gray-900">Places to visit in {{ $city->translate('name') }}</h2>
            </div>
            <div class="flex items-center gap-2">
        ...
        <div class="swiper city-highlights-swiper overflow-visible">
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide">
                    <a href="{{ localized_route('cities.highlights.show', ['city' => $city->slug, 'highlight' => $highlight->slug]) }}" class="group block relative rounded-xl overflow-hidden bg-gray-200" style="aspect-ratio: 4/3;">
                        @if($highlight->image_url)
                            <img src="{{ $highlight->image_url }}" alt="{{ $highlight->translate('title') }}" class="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy">
                        @endif
                        <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent"></div>
                        <div class="absolute bottom-3 left-3 right-3">
                            <h3 class="font-bold text-base drop-shadow line-clamp-2" style="color: #fff !important;">{{ $highlight->translate('title') }}</h3>
                        </div>
                    </a>
                </div>
                @endforeach
    ...
    <section class="mb-14">
        <div class="flex items-end justify-between mb-8">
            <div>
                ...
                <h2 class="text-2xl md:text-3xl font-bold text-gray-900">Tours in {{ $city->translate('name') }}</h2>
            </div>
            <div class="flex items-center gap-2">
                ...
                <a href="{{ localized_route('tours.search', ['city' => $city->slug]) }}" class="text-sm font-medium text-brand-navy hover:text-brand-navy transition hidden sm:block">
                    View all tours &rarr;
                </a>
            </div>
        </div>
        ...
        <div class="mt-6 text-center sm:hidden">
            <a href="{{ localized_route('tours.index', ['city' => $city->slug]) }}" class="text-sm font-medium text-brand-navy hover:text-brand-navy transition">
                View all tours &rarr;
            </a>
        </div>
    </section>
    ...
    <section>
        <div class="flex items-end justify-between mb-8">
            <div>
                ...
                <h2 class="text-2xl md:text-3xl font-bold text-gray-900">Hotels in {{ $city->translate('name') }}</h2>
            </div>
            <div class="flex items-center gap-2 sm:hidden">
        ...
        <div class="hidden sm:block" x-data="{ displayed: 8 }">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                ...
                <a href="{{ localized_route('hotels.show', ['slug' => $hotel->slug]) }}" x-show="{{ $loop->index }} < displayed" x-cloak
                    ...
                    <div class="aspect-[4/3] bg-gray-200 overflow-hidden">
                        @if($hotel->image_url)
                            <img src="{{ $hotel->image_url }}" alt="{{ $hotel->translate('name') }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy">
                        @else
                            <div class="w-full h-full flex items-center justify-center text-gray-400 text-sm">No image</div>
                        @endif
                    </div>
                    <div class="p-4">
                        <h3 class="font-semibold text-gray-900 group-hover:text-brand-navy transition-colors">{{ $hotel->translate('name') }}</h3>
        ...
        <div class="swiper city-hotels-swiper overflow-visible block sm:!hidden">
            <div class="swiper-wrapper">
                @foreach($city->hotels as $hotel)
                <div class="swiper-slide">
                    <a href="{{ localized_route('hotels.show', ['slug' => $hotel->slug]) }}" class="group block bg-white rounded-xl overflow-hidden border border-gray-100 hover:border-blue-300 hover:shadow-lg transition-all duration-300">
                        <div class="aspect-[4/3] bg-gray-200 overflow-hidden">
                            @if($hotel->image_url)
                                <img src="{{ $hotel->image_url }}" alt="{{ $hotel->translate('name') }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy">
                            @else
